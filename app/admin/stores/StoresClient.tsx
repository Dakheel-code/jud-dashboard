...
function StoresPageContent() {
  ...
  const filteredStores = useMemo(() => {
    ...
    return stores.filter(store => {
      ...
      const matchesSearch = !q || 
        ...
        store.store_url.toLowerCase().includes(q) ||
  ...
  const fetchData = useCallback(async () => {
    try {
      const storesRes = await fetch('/api/admin/stores?limit=200', { cache: 'no-store' });
  ...
  const openEditModal = useCallback((store: StoreWithProgress) => {
    setEditingStore(store);
    setShowAddModal(true);
  }, []);

  const openDeleteModal = useCallback((storeId: string, storeName: string) => {
