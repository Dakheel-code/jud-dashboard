...
function StoreDetailsContent() {
  ...
  const replaceWhatsappVariables = (message: string, taskTitle: string, categoryName: string) => {
    if (!message) return '';
    
    return message
      .replace(/\{\{store_name\}\}/g, storeData?.store_name || '')
      .replace(/\{\{store_url\}\}/g, storeData?.store_url || '')
      .replace(/\{\{task_name\}\}/g, taskTitle)
      .replace(/\{\{section_name\}\}/g, categoryName)
      .replace(/\{\{assigned_to\}\}/g, storeData?.account_manager?.name || '')
      .replace(/\{\{due_date\}\}/g, new Date().toLocaleDateString('ar-SA'))
      .replace(/\{\{company_name\}\}/g, storeData?.owner_name || '');
  };

  // دالة لفتح واتساب مع الرسالة المخصصة
  const openWhatsappWithMessage = (task: any, categoryName: string) => {
    if (!task.whatsapp_message || !storeData?.owner_phone) return;
    
    const message = replaceWhatsappVariables(task.whatsapp_message, task.title, categoryName);
    const phone = storeData.owner_phone.replace(/[^0-9]/g, '');
