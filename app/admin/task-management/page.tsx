...
function TasksManagementContent() {
  ...
  const [editedCategoryWhatsapp, setEditedCategoryWhatsapp] = useState('');
  ...
  // المتغيرات المتاحة لرسائل واتساب
  ...
  useEffect(() => {
    ...
    const savedMessages = localStorage.getItem('categoryWhatsappMessages');
    if (savedMessages) {
      setCategoryWhatsappMessages(JSON.parse(savedMessages));
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEditingTask(null);
        setFormData({ title: '', description: '', category: categories[0], order_index: 0, whatsapp_message: '' });
      } else {
  ...
  const openEditCategoryModal = (category: string) => {
    ...
    setEditedCategoryWhatsapp(categoryWhatsappMessages[category] || '');
  ...
  const handleEditCategory = async () => {
    ...
    // حفظ رسالة واتساب للقسم
    setCategoryWhatsappMessages(prev => ({
      ...prev,
      [editedCategoryName.trim() || editingCategory]: editedCategoryWhatsapp
    }));
    ...
    localStorage.setItem('categoryWhatsappMessages', JSON.stringify(updatedMessages));
    
    // إذا لم يتغير اسم القسم، نغلق المودال فقط
    if (!editedCategoryName.trim() || editedCategoryName.trim() === editingCategory) {
      setShowEditCategoryModal(false);
      setEditingCategory(null);
      setEditedCategoryName('');
      setEditedCategoryWhatsapp('');
      setResultModalType('success');
      setResultModalMessage('تم حفظ رسالة واتساب للقسم بنجاح!');
      setShowResultModal(true);
      return;
    }
    ...
    try {
      ...
      await fetchTasks();
      setResultModalType('success');
      setResultModalMessage('تم تعديل القسم بنجاح!');
    } catch (err) {
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 w-full lg:w-auto">
            <button
              onClick={() => {
                setEditingTask(null);
                setFormData({ title: '', description: '', category: categories[0] || '', order_index: tasks.length, whatsapp_message: '' });
                setShowAddModal(true);
              }}
            ...
            <div key={category} className="bg-purple-950/40  rounded-2xl sm:rounded-3xl shadow-2xl shadow-purple-900/30 p-4 sm:p-6 border border-purple-500/20">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  {/* علامة واتساب للقسم */}
                  {categoryWhatsappMessages[category] && (
                    <span className="text-green-400" title="يوجد رسالة واتساب مخصصة للقسم">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      {task.whatsapp_message && (
                        <span className="text-green-400" title="رسالة واتساب مخصصة">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-purple-950/90  rounded-3xl shadow-2xl p-8 border border-purple-500/30 max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              {/* رسالة واتساب المخصصة للقسم */}
              <div>
                <label className="block text-white mb-2 text-sm font-medium">
                  رسالة واتساب للقسم بالكامل
                  <span className="text-purple-400 text-xs mr-2">(اختياري)</span>
                </label>
                <textarea
                  value={editedCategoryWhatsapp}
                  onChange={(e) => setEditedCategoryWhatsapp(e.target.value)}
                  placeholder="اكتب رسالة واتساب مخصصة لهذا القسم..."
                  rows={3}
                ...
                {/* المتغيرات المتاحة */}
                <div className="mt-2">
                  <p className="text-purple-300 text-xs mb-2">المتغيرات المتاحة (انقر للإضافة):</p>
                  <div className="flex flex-wrap gap-2">
                    {whatsappVariables.map((variable) => (
                      <button
                        key={variable.key}
                        type="button"
                        onClick={() => setEditedCategoryWhatsapp(prev => prev + variable.key)}
                        className="px-2 py-1 text-xs bg-purple-800/50 text-purple-200 rounded-lg hover:bg-purple-700/50 transition-colors"
                      >
                        {variable.label}
                      </button>
                    ))}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="اكتب رسالة واتساب مخصصة لهذه المهمة... يمكنك استخدام المتغيرات أدناه"
                  rows={3}
                ...
                {/* المتغيرات المتاحة */}
                <div className="mt-2">
                  <p className="text-purple-400 text-xs mb-2">المتغيرات المتاحة (انقر للإضافة):</p>
                  <div className="flex flex-wrap gap-1">
                    {whatsappVariables.map(v => (
                      <button
                        key={v.key}
                        type="button"
                        onClick={() => setFormData({ ...formData, whatsapp_message: formData.whatsapp_message + v.key })}
                        className="px-2 py-1 text-xs bg-green-500/20 text-green-300 rounded-lg hover:bg-green-500/30 transition-all border border-green-500/30"
                      >
                        {v.label}
                      </button>
                    ))}
