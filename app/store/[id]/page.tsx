      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 py-8 pb-24 space-y-5">
        ...
        <section className="bg-purple-950/40 rounded-2xl border border-purple-500/20 overflow-hidden">
          ...
          {adsLoading ? (
            ...
            <div>
              <div className="grid grid-cols-5 px-4 py-2 text-[10px] text-purple-300/40 border-b border-purple-500/10">
                <span className="text-right col-span-2">المنصة</span>
              ...
              {adsPlatforms.map((p, i) => {
                ...
                return (
                  <div key={i} className="grid grid-cols-5 px-4 py-3 border-b border-purple-500/8 hover:bg-purple-500/5 transition-colors">
                    <div className="col-span-2 flex items-center gap-2 justify-end">
                      <div>
                        <p className={`text-sm font-medium ${pm.color}`}>{pm.label}</p>
                        <p className="text-[10px] text-purple-300/40 truncate max-w-[100px]">{p.account_name} <span className={`inline-block w-1.5 h-1.5 rounded-full ${pm.dot} mr-1`} /></p>
                      </div>
                    </div>
                    <p className="text-center text-sm text-white font-medium self-center">
                      {p.spend > 0 ? p.spend.toLocaleString('ar') : <span className="text-purple-300/30">0</span>}
                    </p>
                    <p className="text-center text-sm text-white font-medium self-center">
                      {p.sales > 0 ? p.sales.toLocaleString('ar') : <span className="text-purple-300/30">0</span>}
                    </p>
                    <p className="text-center text-sm self-center">
                      {p.orders > 0 ? <span className="text-green-400 font-medium">{p.orders.toLocaleString('ar')}</span> : <span className="text-purple-300/30">0</span>}
                    </p>
                  </div>
                );
              })}
              {adsSummary && (
                <div className="grid grid-cols-5 px-4 py-3 bg-purple-500/5 border-t border-purple-500/15">
                  <div className="col-span-2 flex items-center justify-end">
                    <span className="text-xs text-purple-300/60">الإجمالي ({adsPlatforms.length} منصة)</span>
                  </div>
                  <p className="text-center text-sm font-bold text-red-300">{adsSummary.spend.toLocaleString('ar')}</p>
                  <p className="text-center text-sm font-bold text-white">{adsSummary.sales.toLocaleString('ar')}</p>
                  <p className="text-center text-sm font-bold text-green-400">{adsSummary.orders.toLocaleString('ar')}</p>
                </div>
              )}
            </div>
          )}
