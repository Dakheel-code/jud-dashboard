...
export default function MetaAdsCard({ storeId, embedded = false, onSummaryLoaded, externalPreset }: Props) {
  ...
  const renderBody = () => {
    ...
    if (connection.status === 'error' || needsReauth) return (
      <div className="p-8 text-center">
        ...
        <p className="text-red-400 font-semibold mb-1">انتهت صلاحية ربط Meta</p>
        <p className="text-purple-400/60 text-xs mb-5">يجب إعادة تصريح التطبيق للوصول لبيانات الإعلانات</p>
        <a href={`/api/meta/connect?storeId=${storeId}`}
          className="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-indigo-500/20">
          <MetaIcon className="w-4 h-4" /> إعادة ربط Meta
        </a>
      </div>
    );
